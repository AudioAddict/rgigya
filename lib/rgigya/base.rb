...
module RGigya
  ...
  class << self
    ...
    def required_parameters
      params =  "apiKey=#{Rack::Utils.escape(@@api_key)}"
      params += "&secret=#{Rack::Utils.escape(@@api_secret)}"
      params += "&format=json"
    end
      ...
      if(http_method == "GET")
        ...
        if(options)
          options.each do |key,value|
            url += "&#{key}=#{Rack::Utils.escape(value.to_s)}"
          end
        end
      else
    ...
    def params_with_signature(request_uri,params)
        ...
        normalized_url = Rack::Utils.escape(request_uri)

        query_string = Rack::Utils.escape(prepare_for_signature(params).to_query)
